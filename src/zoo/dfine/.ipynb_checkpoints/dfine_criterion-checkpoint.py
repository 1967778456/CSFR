...
class DFINECriterion(nn.Module):
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        # # ====== 位置先验 + 蒸馏损失（来自 encoder）====== #
        # if "pos_loss" in outputs and outputs["pos_loss"] is not None:
        #     # 注意：这里不再进 get_loss/weight_dict 那一套循环，所以需要手动乘权重
        #     w = self.weight_dict.get("loss_pos_prior", 1.0)
        #     losses["loss_pos_prior"] = outputs["pos_loss"] * w
