...
class HybridEncoder(nn.Module):
    ...
    def __init__(
        ...
        # 只在 top-down 最后一次融合（即生成最高分辨率那层：C3 对应的 fused）使用
        self.fpn_fuse_proj_c3 = nn.Sequential(
            nn.Conv2d(hidden_dim * 2, hidden_dim, 1, bias=False),
            nn.BatchNorm2d(hidden_dim),
            nn.ReLU(inplace=True),
        )
        self.nade_fpn_c3 = NADE(in_channels=hidden_dim, num_heads=nhead, window_size=10)  # 80/72 可整除用自适应版的话更稳
    ...
    def forward(self, feats):
        ...
        for idx in range(len(proj_feats) - 1, 0, -1):
            ...
            feat_high = self.lateral_convs[len(proj_feats) - 1 - idx](feat_high)
            ...
            alpha = self.alpha_fpn[len(proj_feats) - 1 - idx](fused)
            fused = fused * (1.0 + alpha * entropy_map)

         # ✅ 只在融合到 C3 的时候启用 NADE
            if (idx - 1) == 0:
                fused_c3 = self.fpn_fuse_proj_c3(fused)   # [B,C,H,W]
                fused_c3 = self.nade_fpn_c3(fused_c3)     # [B,C,H,W]
                # 再拼回 2C 给 fpn_block（最小改动）
                fused = torch.cat([fused_c3, fused_c3], dim=1)  # [B,2C,H,W] 先这么做跑通
            
            inner_outs[0] = self.fpn_blocks[len(proj_feats) - 1 - idx](fused) + self.residual_weight * upsample
